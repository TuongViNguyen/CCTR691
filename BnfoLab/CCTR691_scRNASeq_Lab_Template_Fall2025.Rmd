...
> There are roughly triple the amount of cells, might be the wrong sample or extra samples were processed. The tSNE projection is not well seperated and so the analysis to identify the different cell populations should be repeated.
...
```{r setup}
knitr::opts_knit$set(root.dir = "~/CCTR691_scRNASeq")
```
...
> Orginal identification, nCOUNT_RNA, nFEATURE_RNA, active assay 
...
> There are 2575 samples. The dimention represents genes and cells with 19,612 representing the estimated number of genes. 
...
> The majority of cells that made the cut off were in the lowest range displayed on the nFeature and nCOUNT plot, so the cell contributing the least to both categories.  
...
> The title shows the different principle components, across the same DEG 
...
> Increase in PC number makes the seperation in the sample less distinct, the sample starts to merge with no obvious patterns.
...
#Create a tSNE plot using the 3 dimensions.
whim_filtered <- RunTSNE(whim_filtered, dims = 1:3)
DimPlot(whim_filtered, reduction = "tsne")

```

> There are minor differences between 5 and 3 dimentions, the seperation in the data remains, but it is not drastic 

How about only the first dimension? 

```{r}

#Create a tSNE plot using the 1 dimensions.
whim_filtered <- RunTSNE(whim_filtered, dims = 1:1)
DimPlot(whim_filtered, reduction = "tsne")

```

> Instead of clustered sections, the different points now run in a string pattern with in conclusive seperations. 

What happens when you use more dimensions?

```{r}

#Create a tSNE plot using the 10 dimensions.
whim_filtered <- RunTSNE(whim_filtered, dims = 1:10)
DimPlot(whim_filtered, reduction = "tsne")

```

> With a dimention of 10, the graph now display a pattern of 2 cluster, but the seperation of the 1 big cluster is not clear. 
...
> log2FC of SCGB2A2:2.7998690; KRT16:4.174405 
